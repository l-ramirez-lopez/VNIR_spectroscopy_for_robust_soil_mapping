...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{requiredpackages <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"resemble"}\NormalTok{,    }
                      \StringTok{"prospectr"}\NormalTok{,   }
                      \StringTok{"clhs"}\NormalTok{,        }
                      \StringTok{"matrixStats"}\NormalTok{, }
                      \StringTok{"doParallel"}\NormalTok{,  }
                      \StringTok{"ggplot2"}\NormalTok{,}
                      \StringTok{"tidyr"}\NormalTok{,}
                      \StringTok{"tidyverse"}\NormalTok{,}
                      \StringTok{"georob"}\NormalTok{, }
...
\NormalTok{\}}
\KeywordTok{lapply}\NormalTok{(requiredpackages, }
       \DataTypeTok{FUN =}\NormalTok{ library, }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Apply standard normal variate }
...
\NormalTok{## standardize the socres}
...
\NormalTok{## compute the max and min of each score for the limits of the density estimations}
...
\NormalTok{## compute the mean and sd of each score for the comparisons with the samples}
...
\NormalTok{## matrix where the density values will be stored}
...
\NormalTok{## Kernel density estimates (KDE) of each component }
...
\NormalTok{## Create the vector of different sample set sizes to be tested}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Sample with LHS (latin hypercube sampling)}

\NormalTok{## These three nested loops might take a while}
\NormalTok{## (aprox. 16 min per repetition, i.e. a bit more than a couple of hours }
\NormalTok{## for the whole thing)}
\NormalTok{## (for reducing the computation time the loops}
\NormalTok{## can be vectorized using the apply family of functions. }
\NormalTok{## Furtheromre parallelization of the loop for the repetitions}
\NormalTok{## can also be applied)}
\NormalTok{## We present the computations with nested loops for interpretability }
\NormalTok{## reasons}

\NormalTok{repetitions <-}\StringTok{ }\DecValTok{10}

\NormalTok{## root name for the results of each iteration}
\NormalTok{filerootname <-}\StringTok{ "6pcs_resultsclhs_rep"}

\NormalTok{## Create the data.frame where the results will be stored }
\NormalTok{## at each iteration}
...
\NormalTok{    ## Compute the KDEs of each PC}
...
\NormalTok{      ## use the same bandwidth (bw) as in the whole set of candidates}
...
\NormalTok{    ## write the results obtained so far...}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Specify a file name for the file where the final results will be stored}
...
\NormalTok{  ## write a table with the final results }
  \ControlFlowTok{if}\NormalTok{(iter }\OperatorTok{==}\StringTok{ }\KeywordTok{length}\NormalTok{(nmsrepsclhs))\{}
\NormalTok{    final.clhs <-}\StringTok{ }\NormalTok{final.clhs}\OperatorTok{/}\NormalTok{iter}
    \KeywordTok{write.table}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ final.clhs, }
...
\NormalTok{## Compute the standard devitions of the msd results}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Compute the maximum and minimum of each score for the limits of the}
\NormalTok{## density estimations}
\NormalTok{max.sc <-}\StringTok{ }\KeywordTok{colMins}\NormalTok{(pcaall}\OperatorTok{$}\NormalTok{scores.std)}
\NormalTok{min.sc <-}\StringTok{ }\KeywordTok{colMaxs}\NormalTok{(pcaall}\OperatorTok{$}\NormalTok{scores.std)}

\NormalTok{## Compute the mean and standard deviation of each score for the}
\NormalTok{## comparisons with the samples}
\NormalTok{mean.sc <-}\StringTok{ }\KeywordTok{colMeans}\NormalTok{(pcaall}\OperatorTok{$}\NormalTok{scores.std)}
\NormalTok{sd.sc <-}\StringTok{ }\KeywordTok{colSds}\NormalTok{(pcaall}\OperatorTok{$}\NormalTok{scores.std)}

\NormalTok{## Set a number of solutions to test}
\NormalTok{solutions <-}\StringTok{ }\DecValTok{10}

\NormalTok{## object where the sample indices of the different solutions }
\NormalTok{## will be stored}
\NormalTok{calidx <-}\StringTok{ }\OtherTok{NULL} 

\NormalTok{## object where the msds will be stored}
...
\NormalTok{  ## set seed for the random number generation}
...
\NormalTok{  ## sample the ith solution  }
\NormalTok{  i.calidx <-}\StringTok{ }\KeywordTok{clhs}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{as.data.frame}\NormalTok{(pcaall}\OperatorTok{$}\NormalTok{scores.std),}
                   \DataTypeTok{size =}\NormalTok{ ocss, }\DataTypeTok{iter =} \DecValTok{10000}\NormalTok{)}
  
\NormalTok{  ## store the indices of the selected samples }
\NormalTok{  calidx[[i]] <-}\StringTok{ }\NormalTok{i.calidx}
  
\NormalTok{  ## Compute the KDEs}
\NormalTok{  j.sc.dens <-}\StringTok{ }\NormalTok{msd.sc  <-}\StringTok{ }\NormalTok{sc.dens}
  \ControlFlowTok{for}\NormalTok{(j }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(min.sc))\{}
\NormalTok{    j.sc.dens[, j }\OperatorTok{+}\StringTok{ }\DecValTok{1}\NormalTok{] <-}\StringTok{ }\KeywordTok{density}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ pcaall}\OperatorTok{$}\NormalTok{scores.std[i.calidx,j],}
                                  \DataTypeTok{bw =}\NormalTok{ d.bandwidths[j],}
                                  ...
                                  \DataTypeTok{kernel =} \StringTok{"gaussian"}\NormalTok{)}\OperatorTok{$}\NormalTok{y}
\NormalTok{  \}}
\NormalTok{  ## compute the msds}
\NormalTok{  results.clhs.ocss[i] <-}\StringTok{ }\KeywordTok{mean}\NormalTok{(}\KeywordTok{colMeans}\NormalTok{((j.sc.dens[,}\OperatorTok{-}\DecValTok{1}\NormalTok{] }\OperatorTok{-}\StringTok{ }\NormalTok{sc.dens[,}\OperatorTok{-}\DecValTok{1}\NormalTok{])}\OperatorTok{^}\DecValTok{2}\NormalTok{, }\DataTypeTok{na.rm =}\NormalTok{ T))}
\NormalTok{\}}

\NormalTok{## Identify the index of the best group}
\KeywordTok{plot}\NormalTok{(results.clhs.ocss)}

\NormalTok{## best solution }
\KeywordTok{which.min}\NormalTok{(results.clhs.ocss)}

\NormalTok{## get the indices of the samples in the final solution}
\NormalTok{calibration.idx <-}\StringTok{ }\NormalTok{calidx[[}\KeywordTok{which.min}\NormalTok{(results.clhs.ocss)]]}

\NormalTok{## get the IDs of the samples in the final solution}
...
\NormalTok{## objects to be removed}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## The above equation can be simply applied in two lines of code}

\NormalTok{## Calibration dataset}
\NormalTok{## alr for silt contnet}
...
\NormalTok{## alr for clay contnet}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Add to the train data a variable indicating to what sampling point}
\NormalTok{## each sample belongs to. This variable will be used internally }
\NormalTok{## by the mbl function during the internal validations. }
...
\NormalTok{## Dissimilarity metric: partial least squares (pls)}
\NormalTok{dmetric <-}\StringTok{ "pls"}

\NormalTok{## Threshold distances to be tested}
\NormalTok{## Note: the number of threshold distances tested here is large, }
\NormalTok{## it could be reduced}
...
\NormalTok{## Define the minimum and maximum number of neighbors that must be retained}
\NormalTok{## for each local model.}
\NormalTok{## Example: If with a given threshold distance only 70 neighbors are}
\NormalTok{## selected, then the function will be fored to take the especified minimum}
\NormalTok{## number of neighbors to be included in the model. In this case 120. }
\NormalTok{## We also set that the maximum number of neighbors that can be}
\NormalTok{## included can be all the samples in the calibration set.}
...
\NormalTok{## Regression method: Weighted average pls (wapls)}
\NormalTok{rmethod <-}\StringTok{ "wapls1"}

\NormalTok{## set the maximum and minimum number of pls factors }
\NormalTok{## for the local wapls regressions}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Run the MBL and find the optimal threshold distance }
\NormalTok{## for neighbor selection and predict Ca for validation}
\NormalTok{## - MBL fits a wapls model to each sample in Xu using }
\NormalTok{##   the calibration samples \{Yr,Xr\}}
\NormalTok{## - The internal validation predictions }
\NormalTok{##   these are predictions of the nearest samples}
\NormalTok{##   of each Xu found in Xr - Nearest neighbor validation (NNv) -}
\NormalTok{sbl_ca <-}\StringTok{ }\KeywordTok{mbl}\NormalTok{(}\DataTypeTok{Yr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{Ca ,}
              \DataTypeTok{Xr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{spc,}
              \DataTypeTok{Xu =}\NormalTok{ valida}\OperatorTok{$}\NormalTok{spc,}
              \DataTypeTok{mblCtrl =}\NormalTok{ ctrl,}
              \DataTypeTok{group =}\NormalTok{ train}\OperatorTok{$}\NormalTok{samplegroup,}
              \DataTypeTok{dissUsage =} \StringTok{"none"}\NormalTok{,}
              \DataTypeTok{k.diss =}\NormalTok{ diss2test, }
              \DataTypeTok{k.range =}\NormalTok{ kminmax,}
              \DataTypeTok{pls.c =}\NormalTok{ pls.f,}
              \DataTypeTok{method =}\NormalTok{ rmethod)}
\NormalTok{sbl_ca}

\NormalTok{## Best threshold distance}
...
\NormalTok{## Get the predicted values for the validation set}
...
\NormalTok{## Residual variances}
\NormalTok{## layer A}
\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerA[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "Ca"}\NormalTok{] <-}\StringTok{ }\KeywordTok{var}\NormalTok{(valida}\OperatorTok{$}\NormalTok{Ca[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'A'}\NormalTok{] }\OperatorTok{-}\StringTok{ }\NormalTok{pred_valCa[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'A'}\NormalTok{])}
\NormalTok{## layer B}
\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerB[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "Ca"}\NormalTok{] <-}\StringTok{ }\KeywordTok{var}\NormalTok{(valida}\OperatorTok{$}\NormalTok{Ca[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'B'}\NormalTok{] }\OperatorTok{-}\StringTok{ }\NormalTok{pred_valCa[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'B'}\NormalTok{])}
\NormalTok{## layers A and B}
\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layersAB[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "Ca"}\NormalTok{] <-}\StringTok{ }\KeywordTok{var}\NormalTok{(valida}\OperatorTok{$}\NormalTok{Ca }\OperatorTok{-}\StringTok{ }\NormalTok{pred_valCa)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sbl_alrclay <-}\StringTok{ }\KeywordTok{mbl}\NormalTok{(}\DataTypeTok{Yr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{alr_Clay, }
                   \DataTypeTok{Xr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{spc, }
                   \DataTypeTok{Xu =}\NormalTok{ valida}\OperatorTok{$}\NormalTok{spc,}
                   \DataTypeTok{mblCtrl =}\NormalTok{ ctrl,}
                   \DataTypeTok{group =}\NormalTok{ train}\OperatorTok{$}\NormalTok{samplegroup,}
...
\CommentTok{# RMSE}
\NormalTok{valrestuls}\OperatorTok{$}\NormalTok{RMSE[valrestuls}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_clay"}\NormalTok{] <-}\StringTok{ }\KeywordTok{mean}\NormalTok{((valida}\OperatorTok{$}\NormalTok{alr_Clay }\OperatorTok{-}\StringTok{ }\NormalTok{pred_val_alrclay)}\OperatorTok{^}\DecValTok{2}\NormalTok{)}\OperatorTok{^}\FloatTok{0.5}
\CommentTok{# ME}
...
\NormalTok{## Residual variances}
\NormalTok{## layer A}
\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerA[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Clay"}\NormalTok{] <-}\StringTok{ }\KeywordTok{var}\NormalTok{(valida}\OperatorTok{$}\NormalTok{alr_Clay[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'A'}\NormalTok{] }\OperatorTok{-}\StringTok{ }\NormalTok{pred_val_alrclay[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'A'}\NormalTok{])}
\NormalTok{## layer B}
\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerB[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Clay"}\NormalTok{] <-}\StringTok{ }\KeywordTok{var}\NormalTok{(valida}\OperatorTok{$}\NormalTok{alr_Clay[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'B'}\NormalTok{] }\OperatorTok{-}\StringTok{ }\NormalTok{pred_val_alrclay[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'B'}\NormalTok{])}
\NormalTok{## layers A and B}
\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layersAB[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Clay"}\NormalTok{] <-}\StringTok{ }\KeywordTok{var}\NormalTok{(valida}\OperatorTok{$}\NormalTok{alr_Clay }\OperatorTok{-}\StringTok{ }\NormalTok{pred_val_alrclay)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sbl_alrsilt <-}\StringTok{ }\KeywordTok{mbl}\NormalTok{(}\DataTypeTok{Yr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{alr_Silt, }
                   \DataTypeTok{Xr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{spc, }
                   \DataTypeTok{Xu =}\NormalTok{ valida}\OperatorTok{$}\NormalTok{spc,}
                   \DataTypeTok{mblCtrl =}\NormalTok{ ctrl,}
                   \DataTypeTok{group =}\NormalTok{ train}\OperatorTok{$}\NormalTok{samplegroup,}
...
\CommentTok{# RMSE}
\NormalTok{valrestuls}\OperatorTok{$}\NormalTok{RMSE[valrestuls}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_silt"}\NormalTok{] <-}\StringTok{ }\KeywordTok{mean}\NormalTok{((valida}\OperatorTok{$}\NormalTok{alr_Clay }\OperatorTok{-}\StringTok{ }\NormalTok{pred_val_alrsilt)}\OperatorTok{^}\DecValTok{2}\NormalTok{)}\OperatorTok{^}\FloatTok{0.5}
\CommentTok{# ME}
...
\NormalTok{## Residual variances}
\NormalTok{## layer A}
\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerA[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Silt"}\NormalTok{] <-}\StringTok{ }\KeywordTok{var}\NormalTok{(valida}\OperatorTok{$}\NormalTok{alr_Silt[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'A'}\NormalTok{] }\OperatorTok{-}\StringTok{ }\NormalTok{pred_val_alrsilt[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'A'}\NormalTok{])}
\NormalTok{## layer B}
\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerB[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Silt"}\NormalTok{] <-}\StringTok{ }\KeywordTok{var}\NormalTok{(valida}\OperatorTok{$}\NormalTok{alr_Silt[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'B'}\NormalTok{] }\OperatorTok{-}\StringTok{ }\NormalTok{pred_val_alrsilt[valida}\OperatorTok{$}\NormalTok{layer }\OperatorTok{==}\StringTok{ 'B'}\NormalTok{])}
\NormalTok{## layers A and B}
\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layersAB[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Silt"}\NormalTok{] <-}\StringTok{ }\KeywordTok{var}\NormalTok{(valida}\OperatorTok{$}\NormalTok{alr_Silt }\OperatorTok{-}\StringTok{ }\NormalTok{pred_val_alrsilt)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{vnirpredictions <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{Ca_spec =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(pred)),}
                              \DataTypeTok{alr_Clay_spec =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(pred)),}
                              \DataTypeTok{alr_Silt_spec =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(pred)))}

\NormalTok{vnirpredictions <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{Ca_spec =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(pred)),}
                              \DataTypeTok{alr_Clay_spec =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(pred)),}
                              \DataTypeTok{alr_Silt_spec =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(pred)))}

\NormalTok{## Add additional relevant information from the original prediction set}
\NormalTok{vnirpredictions <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(pred[,}\KeywordTok{c}\NormalTok{(}\StringTok{"ID"}\NormalTok{, }
                                 \StringTok{"POINT_X"}\NormalTok{, }
                                 \StringTok{"POINT_Y"}\NormalTok{,}
                                 \StringTok{"set"}\NormalTok{,}
...
\NormalTok{## Re-encode the set variable to "prediction"}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Predict Ca in the prediction set based on the optimized threshold}
\CommentTok{# distance for neighbor selection}
\NormalTok{sbl_ca_pred <-}\StringTok{ }\KeywordTok{mbl}\NormalTok{(}\DataTypeTok{Yr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{Ca, }
                   \DataTypeTok{Xr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{spc, }
                   \DataTypeTok{Xu =}\NormalTok{ pred}\OperatorTok{$}\NormalTok{spc,}
                   \DataTypeTok{mblCtrl =}\NormalTok{ ctrl,}
                   \DataTypeTok{group =}\NormalTok{ train}\OperatorTok{$}\NormalTok{samplegroup,}
                   \DataTypeTok{dissUsage =} \StringTok{"none"}\NormalTok{,}
                   \DataTypeTok{k.diss =}\NormalTok{ best.kdiss.ca, }
                   \DataTypeTok{k.range =}\NormalTok{ kminmax,}
                   \DataTypeTok{pls.c =}\NormalTok{ pls.f,}
                   \DataTypeTok{method =}\NormalTok{ rmethod)}
\NormalTok{## get the predicted values and store them in nirpredictions}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sbl_alrclay_pred <-}\StringTok{ }\KeywordTok{mbl}\NormalTok{(}\DataTypeTok{Yr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{alr_Clay, }
                        \DataTypeTok{Xr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{spc, }
                        \DataTypeTok{Xu =}\NormalTok{ pred}\OperatorTok{$}\NormalTok{spc,}
                        \DataTypeTok{mblCtrl =}\NormalTok{ ctrl,}
                        \DataTypeTok{group =}\NormalTok{ train}\OperatorTok{$}\NormalTok{samplegroup,}
                        \DataTypeTok{dissUsage =} \StringTok{"none"}\NormalTok{,}
                        \DataTypeTok{k.diss =}\NormalTok{ best.kdiss.alrclay, }
                        \DataTypeTok{k.range =}\NormalTok{ kminmax,}
                        \DataTypeTok{pls.c =}\NormalTok{ pls.f,}
                        \DataTypeTok{method =}\NormalTok{ rmethod)}
\NormalTok{## get the predicted values and store them in nirpredictions}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sbl_alrsilt_pred <-}\StringTok{ }\KeywordTok{mbl}\NormalTok{(}\DataTypeTok{Yr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{alr_Silt, }
                        \DataTypeTok{Xr =}\NormalTok{ train}\OperatorTok{$}\NormalTok{spc, }
                        \DataTypeTok{Xu =}\NormalTok{ pred}\OperatorTok{$}\NormalTok{spc,}
                        \DataTypeTok{mblCtrl =}\NormalTok{ ctrl,}
                        \DataTypeTok{group =}\NormalTok{ train}\OperatorTok{$}\NormalTok{samplegroup,}
                        \DataTypeTok{dissUsage =} \StringTok{"none"}\NormalTok{,}
                        \DataTypeTok{k.diss =}\NormalTok{ best.kdiss.alrsilt, }
                        \DataTypeTok{k.range =}\NormalTok{ kminmax,}
                        \DataTypeTok{pls.c =}\NormalTok{ pls.f,}
                        \DataTypeTok{method =}\NormalTok{ rmethod)}
\NormalTok{## get the predicted values and store them in nirpredictions}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## samples for the set "prediction"}
\NormalTok{vnirpredictions}

\NormalTok{## samples for the set "train"}
\NormalTok{vnirtrain <-}\StringTok{ }\NormalTok{train[ ,}\KeywordTok{c}\NormalTok{(}\StringTok{"ID"}\NormalTok{,}\StringTok{"POINT_X"}\NormalTok{,}\StringTok{"POINT_Y"}\NormalTok{,}\StringTok{"set"}\NormalTok{, }\StringTok{"Ca"}\NormalTok{, }\StringTok{"Clay"}\NormalTok{, }\StringTok{"Silt"}\NormalTok{, }\StringTok{"Sand"}\NormalTok{, }\StringTok{"alr_Clay"}\NormalTok{,}\StringTok{"alr_Silt"}\NormalTok{)]}
...
\NormalTok{## samples for the set "validation"}
\NormalTok{vnirvalidation <-}\StringTok{ }\NormalTok{valida[ ,}\KeywordTok{c}\NormalTok{(}\StringTok{"ID"}\NormalTok{,}\StringTok{"POINT_X"}\NormalTok{,}\StringTok{"POINT_Y"}\NormalTok{,}\StringTok{"set"}\NormalTok{, }\StringTok{"Ca"}\NormalTok{,}\StringTok{"Clay"}\NormalTok{, }\StringTok{"Silt"}\NormalTok{, }\StringTok{"Sand"}\NormalTok{, }\StringTok{"alr_Clay"}\NormalTok{,}\StringTok{"alr_Silt"}\NormalTok{)]}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{vniraugmented <-}\StringTok{ }\KeywordTok{rbind}\NormalTok{(vnirtrain, }
\NormalTok{                       vnirpredictions,}
\NormalTok{                       vnirvalidation)}
...
\NormalTok{## Reorganize the variables}
\NormalTok{vniraugmented <-}\StringTok{ }\NormalTok{vniraugmented[,}\KeywordTok{c}\NormalTok{(}\StringTok{"ID"}\NormalTok{, }
                                  \StringTok{"POINT_X"}\NormalTok{, }
                                  \StringTok{"POINT_Y"}\NormalTok{, }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Names of the properties}
\NormalTok{props <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Ca"}\NormalTok{,}
           \StringTok{"Clay"}\NormalTok{,}
           \StringTok{"Silt"}\NormalTok{,}
           \StringTok{"Sand"}\NormalTok{,}
           \StringTok{"alr_Clay"}\NormalTok{,}
...
\NormalTok{## Compute the statistics: mean, standard deviation and }
\NormalTok{##  the quantiles ("0%", "25%", "50%", "75%" and"100%")}
\NormalTok{statsprops <-}\StringTok{ }\KeywordTok{aggregate}\NormalTok{(vniraugmented[, props],}
                        \DataTypeTok{by =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{set =}\NormalTok{ vniraugmented}\OperatorTok{$}\NormalTok{set,}
                                  \DataTypeTok{layer =}\NormalTok{ vniraugmented}\OperatorTok{$}\NormalTok{layer),}
                        \DataTypeTok{FUN =} \ControlFlowTok{function}\NormalTok{(x)\{}
                          \KeywordTok{c}\NormalTok{(}\DataTypeTok{mean =} \KeywordTok{mean}\NormalTok{(}\KeywordTok{as.matrix}\NormalTok{(x), }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{),}
                            \DataTypeTok{sd =} \KeywordTok{sd}\NormalTok{(}\KeywordTok{as.matrix}\NormalTok{(x), }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{),}
                            \KeywordTok{quantile}\NormalTok{(x, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{))}
\NormalTok{                        \})}

\NormalTok{## Reorganize the object containing the results of the statistics}
\NormalTok{statsprops <-}\StringTok{ }\KeywordTok{lapply}\NormalTok{(props, }
                     \DataTypeTok{FUN =} \ControlFlowTok{function}\NormalTok{(x, object, ids)\{}
\NormalTok{                       object <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(object[,keep], }
                                       \KeywordTok{as.data.frame}\NormalTok{(statsquant[[x]]))}
                       
\NormalTok{                     \}, }
                     \DataTypeTok{object =}\NormalTok{ statsprops,}
                     \DataTypeTok{ids =} \KeywordTok{c}\NormalTok{(}\StringTok{"set"}\NormalTok{, }\StringTok{"layer"}\NormalTok{))}
...
\NormalTok{## split the data sets by layer and by spatial fit and spatial validation}
...
\KeywordTok{plot}\NormalTok{(vallayera}\OperatorTok{$}\NormalTok{POINT_X, vallayera}\OperatorTok{$}\NormalTok{POINT_Y,}
     \DataTypeTok{xlab =} \StringTok{"X"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"Y"}\NormalTok{)}
\KeywordTok{points}\NormalTok{(fitlayera}\OperatorTok{$}\NormalTok{POINT_X, fitlayera}\OperatorTok{$}\NormalTok{POINT_Y, }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Check the sample variogram}
\NormalTok{vario_Ca_lab_a <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayera}\OperatorTok{$}\NormalTok{Ca,}
                                   \DataTypeTok{locations =}\NormalTok{ fitlayera[,}\KeywordTok{c}\NormalTok{(}\StringTok{"POINT_X"}\NormalTok{, }\StringTok{"POINT_Y"}\NormalTok{)], }
                                   \DataTypeTok{lag.dist.def =}\NormalTok{ lagdist, }
                                   \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_Ca_lab_a}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_Ca_lab_a}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_Ca_lab_a}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"Ca, laboratory - Layer A"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_Ca_lab_a <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \DecValTok{135}\NormalTok{, }
                     \DataTypeTok{nugget =} \DecValTok{10}\NormalTok{, }
                     \DataTypeTok{scale =} \DecValTok{920}\NormalTok{)}
\NormalTok{## Fit }
\NormalTok{Ca_lab_a <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(Ca }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                   \DataTypeTok{data =}\NormalTok{ fitlayera, }
                   \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                   \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                   \DataTypeTok{param =}\NormalTok{ startp_Ca_lab_a,}
                   \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                   \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
\KeywordTok{summary}\NormalTok{(Ca_lab_a)}

\NormalTok{## Store the variogram parameters}
...
\begin{Highlighting}[]
\NormalTok{## Check the sample variogram}
\NormalTok{vario_alr_Clay_lab_a <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayera}\OperatorTok{$}\NormalTok{alr_Clay,}
                                         \DataTypeTok{locations =}\NormalTok{ fitlayera[,}\KeywordTok{c}\NormalTok{(}\StringTok{"POINT_X"}\NormalTok{, }\StringTok{"POINT_Y"}\NormalTok{)], }
                                         \DataTypeTok{lag.dist.def =}\NormalTok{ lagdist, }
                                         \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_alr_Clay_lab_a}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_alr_Clay_lab_a}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_alr_Clay_lab_a}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"alr(clay) laboratory - Layer A"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_alr_Clay_lab_a <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \FloatTok{0.8}\NormalTok{, }
                           \DataTypeTok{nugget =} \FloatTok{0.3}\NormalTok{, }
                           \DataTypeTok{scale =} \DecValTok{1220}\NormalTok{)}

\NormalTok{## Fit }
\NormalTok{alr_Clay_lab_a <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(alr_Clay }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                         \DataTypeTok{data =}\NormalTok{ fitlayera, }
                         \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                         \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                         \DataTypeTok{param =}\NormalTok{ startp_alr_Clay_lab_a,}
                         \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                         \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\begin{Highlighting}[]
\NormalTok{## Check the sample variogram}
\NormalTok{vario_alr_Silt_lab_a <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayera}\OperatorTok{$}\NormalTok{alr_Silt,}
                                         \DataTypeTok{locations =}\NormalTok{ fitlayera[,}\KeywordTok{c}\NormalTok{(}\StringTok{"POINT_X"}\NormalTok{, }\StringTok{"POINT_Y"}\NormalTok{)], }
                                         \DataTypeTok{lag.dist.def =}\NormalTok{ lagdist, }
                                         \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_alr_Silt_lab_a}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_alr_Silt_lab_a}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_alr_Silt_lab_a}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"alr(silt) laboratory - Layer A"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_alr_Silt_lab_a <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \FloatTok{1.31}\NormalTok{, }
                           \DataTypeTok{nugget =} \FloatTok{0.3}\NormalTok{, }
                           \DataTypeTok{scale =} \DecValTok{812}\NormalTok{)}

\NormalTok{## Fit }
\NormalTok{alr_Silt_lab_a <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(alr_Silt }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                         \DataTypeTok{data =}\NormalTok{ fitlayera, }
                         \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                         \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                         \DataTypeTok{param =}\NormalTok{ startp_alr_Silt_lab_a,}
                         \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                         \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\begin{Highlighting}[]
\NormalTok{## Check the sample variogram}
\NormalTok{vario_Ca_lab_b <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayerb}\OperatorTok{$}\NormalTok{Ca,}
                                   \DataTypeTok{locations =}\NormalTok{ fitlayerb[,}\KeywordTok{c}\NormalTok{(}\StringTok{"POINT_X"}\NormalTok{, }\StringTok{"POINT_Y"}\NormalTok{)], }
                                   \DataTypeTok{lag.dist.def =}\NormalTok{ lagdist, }
                                   \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_Ca_lab_b}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_Ca_lab_b}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_Ca_lab_b}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"Ca, laboratory - Layer B"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_Ca_lab_b <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \DecValTok{127}\NormalTok{, }
                     \DataTypeTok{nugget =} \FloatTok{0.1}\NormalTok{, }
                     \DataTypeTok{scale =} \DecValTok{850}\NormalTok{)}
\NormalTok{## Fit }
\NormalTok{Ca_lab_b <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(Ca }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                   \DataTypeTok{data =}\NormalTok{ fitlayerb, }
                   \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                   \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                   \DataTypeTok{param =}\NormalTok{ startp_Ca_lab_b,}
                   \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                   \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\begin{Highlighting}[]
\NormalTok{## Check the sample variogram}
\NormalTok{vario_alr_Clay_lab_b <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayerb}\OperatorTok{$}\NormalTok{alr_Clay,}
                                         \DataTypeTok{locations =}\NormalTok{ fitlayerb[,}\KeywordTok{c}\NormalTok{(}\StringTok{"POINT_X"}\NormalTok{, }\StringTok{"POINT_Y"}\NormalTok{)], }
                                         \DataTypeTok{lag.dist.def =}\NormalTok{ lagdist, }
                                         \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_alr_Clay_lab_b}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_alr_Clay_lab_b}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_alr_Clay_lab_b}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"alr(clay) laboratory - Layer B"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_alr_Clay_lab_b <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \FloatTok{1.17}\NormalTok{, }
                           \DataTypeTok{nugget =} \FloatTok{0.1}\NormalTok{,}
                           \DataTypeTok{scale =} \DecValTok{973}\NormalTok{, }
                           \DataTypeTok{alpha =} \FloatTok{0.69}\NormalTok{)}
\NormalTok{## Fit }
\NormalTok{alr_Clay_lab_b <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(alr_Clay }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                         \DataTypeTok{data =}\NormalTok{ fitlayerb, }
                         \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                         \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                         \DataTypeTok{param =}\NormalTok{ startp_alr_Clay_lab_b,}
                         \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                         \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\begin{Highlighting}[]
\NormalTok{## Check the sample variogram}
\NormalTok{vario_alr_Silt_lab_b <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayerb}\OperatorTok{$}\NormalTok{alr_Silt,}
                                         \DataTypeTok{locations =}\NormalTok{ fitlayerb[,}\KeywordTok{c}\NormalTok{(}\StringTok{"POINT_X"}\NormalTok{, }\StringTok{"POINT_Y"}\NormalTok{)], }
                                         \DataTypeTok{lag.dist.def =}\NormalTok{ lagdist, }
                                         \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_alr_Silt_lab_b}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_alr_Silt_lab_b}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_alr_Silt_lab_b}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"alr(silt) laboratory - Layer B"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_alr_Silt_lab_b <-}\StringTok{  }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \FloatTok{1.1}\NormalTok{, }
                            \DataTypeTok{nugget =} \FloatTok{0.2}\NormalTok{, }
                            \DataTypeTok{scale =} \DecValTok{423}\NormalTok{)}

\NormalTok{## Fit }
\NormalTok{alr_Silt_lab_b <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(alr_Silt }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                         \DataTypeTok{data =}\NormalTok{ fitlayerb, }
                         \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                         \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                         \DataTypeTok{param =}\NormalTok{ startp_alr_Silt_lab_b,}
                         \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                         \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\NormalTok{vario_Ca_spec_a <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayera}\OperatorTok{$}\NormalTok{Ca_spec,}
                                    \DataTypeTok{locations =}\NormalTok{ fitlayera[,}\KeywordTok{c}\NormalTok{(}\StringTok{"POINT_X"}\NormalTok{, }\StringTok{"POINT_Y"}\NormalTok{)], }
                                    \DataTypeTok{lag.dist.def =}\NormalTok{ lagdist, }
                                    \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_Ca_spec_a}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_Ca_spec_a}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_Ca_spec_a}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"Ca, vis-NIR augmented - Layer A"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_Ca_spec_a <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \DecValTok{112}\NormalTok{, }
                      \DataTypeTok{nugget =} \DecValTok{1}\NormalTok{, }
                      \DataTypeTok{scale =} \DecValTok{1023}\NormalTok{)}
\NormalTok{## Fit }
\NormalTok{Ca_spec_a <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(Ca }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                    \DataTypeTok{data =}\NormalTok{ fitlayera, }
                    \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                    \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                    \DataTypeTok{param =}\NormalTok{ startp_Ca_spec_a,}
                    \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                    \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\NormalTok{vario_alr_Clay_spec_a <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayera}\OperatorTok{$}\NormalTok{alr_Clay_spec,}
                                          ...
                                          \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_alr_Clay_spec_a}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_alr_Clay_spec_a}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_alr_Clay_spec_a}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"alr(clay) vis-NIR augmented - Layer A"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_alr_Clay_spec_a <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \FloatTok{1.134}\NormalTok{,}
                            \DataTypeTok{nugget =} \FloatTok{0.1}\NormalTok{,}
                            \DataTypeTok{scale =} \DecValTok{955}\NormalTok{)}

\NormalTok{## Fit }
\NormalTok{alr_Clay_spec_a <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(alr_Clay_spec }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                          \DataTypeTok{data =}\NormalTok{ fitlayera, }
                          \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                          \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                          \DataTypeTok{param =}\NormalTok{ startp_alr_Clay_spec_a,}
                          \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                          \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\NormalTok{vario_alr_Silt_spec_a <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayera}\OperatorTok{$}\NormalTok{alr_Silt_spec,}
                                          ...
                                          \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_alr_Silt_spec_a}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_alr_Silt_spec_a}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_alr_Silt_spec_a}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"alr(silt) vis-NIR augmented - Layer A"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_alr_Silt_spec_a <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \FloatTok{0.845}\NormalTok{, }
                            \DataTypeTok{nugget =} \FloatTok{0.1}\NormalTok{, }
                            \DataTypeTok{scale =} \DecValTok{1282}\NormalTok{)}

\NormalTok{## Fit }
\NormalTok{alr_Silt_spec_a <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(alr_Silt_spec }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                          \DataTypeTok{data =}\NormalTok{ fitlayera, }
                          \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                          \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                          \DataTypeTok{param =}\NormalTok{ startp_alr_Silt_spec_a,}
                          \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                          \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\begin{Highlighting}[]
\NormalTok{## Check the sample variogram}
\NormalTok{vario_Ca_spec_b <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayerb}\OperatorTok{$}\NormalTok{Ca_spec,}
                                    \DataTypeTok{locations =}\NormalTok{ fitlayerb[,}\KeywordTok{c}\NormalTok{(}\StringTok{"POINT_X"}\NormalTok{, }\StringTok{"POINT_Y"}\NormalTok{)], }
                                    \DataTypeTok{lag.dist.def =}\NormalTok{ lagdist, }
                                    \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_Ca_spec_b}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_Ca_spec_b}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_Ca_spec_b}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"Ca, vis-NIR augmented - Layer B"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_Ca_spec_b <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \DecValTok{142}\NormalTok{, }
                      \DataTypeTok{nugget =} \FloatTok{0.1}\NormalTok{, }
                      \DataTypeTok{scale =} \DecValTok{1025}\NormalTok{)}
\NormalTok{## Fit }
\NormalTok{Ca_spec_b <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(Ca }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                    \DataTypeTok{data =}\NormalTok{ fitlayerb, }
                    \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                    \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                    \DataTypeTok{param =}\NormalTok{ startp_Ca_spec_b,}
                    \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                    \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\NormalTok{vario_alr_Clay_spec_b <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayerb}\OperatorTok{$}\NormalTok{alr_Clay_spec,}
                                          ...
                                          \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_alr_Clay_spec_b}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_alr_Clay_spec_b}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_alr_Clay_spec_b}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"alr(clay) vis-NIR augmented - Layer B"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_alr_Clay_spec_b <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \FloatTok{1.21}\NormalTok{, }
                            \DataTypeTok{nugget =} \FloatTok{0.1}\NormalTok{, }
                            \DataTypeTok{scale =} \DecValTok{1000}\NormalTok{, }
                            \DataTypeTok{alpha =} \FloatTok{0.68}\NormalTok{)}
\NormalTok{## Fit }
\NormalTok{alr_Clay_spec_b <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(alr_Clay_spec }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                          \DataTypeTok{data =}\NormalTok{ fitlayerb, }
                          \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                          \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                          \DataTypeTok{param =}\NormalTok{ startp_alr_Clay_spec_b,}
                          \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                          \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\NormalTok{vario_alr_Silt_spec_b <-}\StringTok{ }\KeywordTok{sample.variogram}\NormalTok{(}\DataTypeTok{object =}\NormalTok{ fitlayerb}\OperatorTok{$}\NormalTok{alr_Silt_spec,}
                                          ...
                                          \DataTypeTok{estimator =} \StringTok{"matheron"}\NormalTok{)}
\NormalTok{## Plot the sample variogram}
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ vario_alr_Silt_spec_b}\OperatorTok{$}\NormalTok{lag.dist, }
     \DataTypeTok{y =}\NormalTok{ vario_alr_Silt_spec_b}\OperatorTok{$}\NormalTok{gamma, }
     \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\KeywordTok{max}\NormalTok{(vario_alr_Silt_spec_b}\OperatorTok{$}\NormalTok{gamma)),}
     \DataTypeTok{xlab =} \StringTok{"lag distance"}\NormalTok{,}
     \DataTypeTok{ylab =} \StringTok{"gamma"}\NormalTok{,}
     \DataTypeTok{pch =} \DecValTok{16}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"red"}\NormalTok{,}
     \DataTypeTok{main =} \StringTok{"alr(silt) vis-NIR augmented - Layer B"}\NormalTok{)}
\KeywordTok{grid}\NormalTok{()}

\NormalTok{## Check the plot above to select some starting values of the }
\NormalTok{## variogram parameters}
\NormalTok{startp_alr_Silt_spec_b <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DataTypeTok{variance =} \FloatTok{1.43}\NormalTok{, }
                            \DataTypeTok{nugget =} \FloatTok{0.1}\NormalTok{, }
                            \DataTypeTok{scale =} \DecValTok{850}\NormalTok{,}
                            \DataTypeTok{alpha =} \FloatTok{0.63}\NormalTok{)}
\NormalTok{## Fit }
\NormalTok{alr_Silt_spec_b <-}\StringTok{ }\KeywordTok{georob}\NormalTok{(alr_Silt_spec }\OperatorTok{~}\StringTok{ }\DecValTok{1}\NormalTok{, }
                          \DataTypeTok{data =}\NormalTok{ fitlayerb, }
                          \DataTypeTok{locations =} \OperatorTok{~}\StringTok{ }\NormalTok{POINT_X }\OperatorTok{+}\StringTok{ }\NormalTok{POINT_Y, }
                          \DataTypeTok{variogram.model =}\NormalTok{ varmodel,}
                          \DataTypeTok{param =}\NormalTok{ startp_alr_Silt_spec_b,}
                          \DataTypeTok{fit.param =}\NormalTok{ fitparam,}
                          \DataTypeTok{tuning.psi =}\NormalTok{ tpsi)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Add the residual variances to the snugget parameter}
\NormalTok{## of the variograms corresponding to the vis-NIR augmented }
\NormalTok{## data}
\NormalTok{## Note that snugget was 0 for all the fitted variograms}
\NormalTok{## Ca (layer A)}
\NormalTok{variogtablea}\OperatorTok{$}\NormalTok{snugget[variogtablea}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "Ca_spec"}\NormalTok{] <-}\StringTok{ }\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerA[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "Ca"}\NormalTok{]}
\NormalTok{## alr(clay) (layer A)}
\NormalTok{variogtablea}\OperatorTok{$}\NormalTok{snugget[variogtablea}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Clay_spec"}\NormalTok{] <-}\StringTok{ }\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerA[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Clay"}\NormalTok{]}
\NormalTok{## alr(silt) (layer A)}
\NormalTok{variogtablea}\OperatorTok{$}\NormalTok{snugget[variogtablea}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Silt_spec"}\NormalTok{] <-}\StringTok{ }\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerA[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Silt"}\NormalTok{]}

\NormalTok{## Ca (layer B)}
\NormalTok{variogtableb}\OperatorTok{$}\NormalTok{snugget[variogtableb}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "Ca_spec"}\NormalTok{] <-}\StringTok{ }\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerB[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "Ca"}\NormalTok{]}
\NormalTok{## alr(clay)  (layer B)}
\NormalTok{variogtableb}\OperatorTok{$}\NormalTok{snugget[variogtableb}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Clay_spec"}\NormalTok{] <-}\StringTok{ }\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerB[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Clay"}\NormalTok{]}
\NormalTok{## alr(silt) (layer B)}
\NormalTok{variogtableb}\OperatorTok{$}\NormalTok{snugget[variogtableb}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Silt_spec"}\NormalTok{] <-}\StringTok{ }\NormalTok{residualvariances}\OperatorTok{$}\NormalTok{layerB[residualvariances}\OperatorTok{$}\NormalTok{Property }\OperatorTok{==}\StringTok{ "alr_Silt"}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## The final variables to be predicted. In the case of }
\NormalTok{## Clay, silt and Sand they are estimated from the }
\NormalTok{## predictions of alr(clay) and alr(silt)}
\NormalTok{bprops <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Ca"}\NormalTok{, }
            \StringTok{"Clay"}\NormalTok{, }
            \StringTok{"Silt"}\NormalTok{, }
            \StringTok{"Sand"}\NormalTok{, }
            \StringTok{"Ca_spec"}\NormalTok{,}
            \StringTok{"Clay_spec"}\NormalTok{, }
            \StringTok{"Silt_spec"}\NormalTok{, }
            \StringTok{"Sand_spec"}\NormalTok{)}

\NormalTok{## a quick function to create columns of a given length (lg)}
...
\NormalTok{## object where the spatial predictions will be stored }
\NormalTok{sppredsvala <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"ID"}\NormalTok{, bprops), }
                      \DataTypeTok{FUN =}\NormalTok{ idfcol, }
                      \DataTypeTok{lg =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(vallayera)))}
...
\NormalTok{sppredsvalb <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"ID"}\NormalTok{, bprops), }
                      \DataTypeTok{FUN =}\NormalTok{ idfcol, }
                      \DataTypeTok{lg =} \KeywordTok{rep}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(vallayerb)))}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred_Ca_lab_a <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(Ca_lab_a, }
                         \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayera), }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## predict alr(clay)}
\NormalTok{pred_alr_Clay_lab_a <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Clay_lab_a, }
                               \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayera), }
                               \DataTypeTok{control =}\NormalTok{ gcntrl)}
\NormalTok{## predict alr(silt)}
\NormalTok{pred_alr_Silt_lab_a <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Silt_lab_a, }
                               \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayera), }
                               \DataTypeTok{control =}\NormalTok{ gcntrl)}

\NormalTok{## Back-transfrom to clay, silt and sand contents}
\NormalTok{dvr.Silt <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(pred_alr_Silt_lab_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{pred }\OperatorTok{+}\StringTok{ }\NormalTok{(}\FloatTok{0.5} \OperatorTok{*}\StringTok{ }\NormalTok{(pred_alr_Silt_lab_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{var.target }\OperatorTok{-}\StringTok{ }\NormalTok{pred_alr_Silt_lab_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{cov.pred.target)))}
\NormalTok{dvr.Clay <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(pred_alr_Clay_lab_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{pred }\OperatorTok{+}\StringTok{ }\NormalTok{(}\FloatTok{0.5} \OperatorTok{*}\StringTok{ }\NormalTok{(pred_alr_Clay_lab_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{var.target }\OperatorTok{-}\StringTok{ }\NormalTok{pred_alr_Clay_lab_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{cov.pred.target)))}
...
\NormalTok{## Store the back-transformed values}
...
\NormalTok{## Estimate the validation parameters for}
\NormalTok{## Clay}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred_Ca_lab_b <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(Ca_lab_b, }
                         \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayerb), }
                         \DataTypeTok{control =}\NormalTok{ gcntrl)}
...
\begin{Highlighting}[]
\NormalTok{## predict alr(clay)}
\NormalTok{pred_alr_Clay_lab_b <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Clay_lab_b, }
                               \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayerb), }
                               \DataTypeTok{control =}\NormalTok{ gcntrl)}
\NormalTok{## predict alr(silt)}
\NormalTok{pred_alr_Silt_lab_b <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Silt_lab_b, }
                               \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayerb), }
                               \DataTypeTok{control =}\NormalTok{ gcntrl)}

\NormalTok{## Back-transfrom to clay, silt and sand contents}
\NormalTok{dvr.Silt <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(pred_alr_Silt_lab_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{pred }\OperatorTok{+}\StringTok{ }\NormalTok{(}\FloatTok{0.5} \OperatorTok{*}\StringTok{ }\NormalTok{(pred_alr_Silt_lab_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{var.target }\OperatorTok{-}\StringTok{ }\NormalTok{pred_alr_Silt_lab_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{cov.pred.target)))}
\NormalTok{dvr.Clay <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(pred_alr_Clay_lab_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{pred }\OperatorTok{+}\StringTok{ }\NormalTok{(}\FloatTok{0.5} \OperatorTok{*}\StringTok{ }\NormalTok{(pred_alr_Clay_lab_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{var.target }\OperatorTok{-}\StringTok{ }\NormalTok{pred_alr_Clay_lab_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{cov.pred.target)))}
\NormalTok{dvn.pred <-}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{dvr.Silt }\OperatorTok{+}\StringTok{ }\NormalTok{dvr.Clay}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred_Ca_spec_a <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(Ca_spec_a, }
                          \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayera), }
                          \DataTypeTok{control =}\NormalTok{ gcntrl,}
...
\begin{Highlighting}[]
\NormalTok{## predict alr(clay)}
\NormalTok{pred_alr_Clay_spec_a <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Clay_spec_a, }
                                \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayera), }
                                \DataTypeTok{control =}\NormalTok{ gcntrl,}
                                \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtablea[variogtablea}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Clay_spec"}\NormalTok{, varparamames]))}
\NormalTok{## predict alr(silt)}
\NormalTok{pred_alr_Silt_spec_a <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Silt_spec_a, }
                                \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayera), }
                                \DataTypeTok{control =}\NormalTok{ gcntrl,}
                                \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtablea[variogtablea}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Silt_spec"}\NormalTok{, varparamames]))}

\NormalTok{## Back-transfrom to clay, silt and sand contents}
\NormalTok{dvr.Silt <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(pred_alr_Silt_spec_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{pred }\OperatorTok{+}\StringTok{ }\NormalTok{(}\FloatTok{0.5} \OperatorTok{*}\StringTok{ }\NormalTok{(pred_alr_Silt_spec_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{var.target }\OperatorTok{-}\StringTok{ }\NormalTok{pred_alr_Silt_spec_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{cov.pred.target)))}
\NormalTok{dvr.Clay <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(pred_alr_Clay_spec_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{pred }\OperatorTok{+}\StringTok{ }\NormalTok{(}\FloatTok{0.5} \OperatorTok{*}\StringTok{ }\NormalTok{(pred_alr_Clay_spec_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{var.target }\OperatorTok{-}\StringTok{ }\NormalTok{pred_alr_Clay_spec_a}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{cov.pred.target)))}
\NormalTok{dvn.pred <-}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{dvr.Silt }\OperatorTok{+}\StringTok{ }\NormalTok{dvr.Clay}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred_Ca_spec_b <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(Ca_spec_b, }
                          \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayerb), }
                          \DataTypeTok{control =}\NormalTok{ gcntrl,}
...
\begin{Highlighting}[]
\NormalTok{## predict alr(clay)}
\NormalTok{pred_alr_Clay_spec_b <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Clay_spec_b, }
                                \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayerb), }
                                \DataTypeTok{control =}\NormalTok{ gcntrl,}
                                \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtableb[variogtableb}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Clay_spec"}\NormalTok{, varparamames]))}
\NormalTok{## predict alr(silt)}
\NormalTok{pred_alr_Silt_spec_b <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Silt_spec_b, }
                                \DataTypeTok{newdata =} \KeywordTok{as.data.frame}\NormalTok{(vallayerb), }
                                \DataTypeTok{control =}\NormalTok{ gcntrl,}
                                \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtableb[variogtableb}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Silt_spec"}\NormalTok{, varparamames]))}

\NormalTok{## Back-transfrom to clay, silt and sand contents}
\NormalTok{dvr.Silt <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(pred_alr_Silt_spec_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{pred }\OperatorTok{+}\StringTok{ }\NormalTok{(}\FloatTok{0.5} \OperatorTok{*}\StringTok{ }\NormalTok{(pred_alr_Silt_spec_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{var.target }\OperatorTok{-}\StringTok{ }\NormalTok{pred_alr_Silt_spec_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{cov.pred.target)))}
\NormalTok{dvr.Clay <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(pred_alr_Clay_spec_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{pred }\OperatorTok{+}\StringTok{ }\NormalTok{(}\FloatTok{0.5} \OperatorTok{*}\StringTok{ }\NormalTok{(pred_alr_Clay_spec_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{var.target }\OperatorTok{-}\StringTok{ }\NormalTok{pred_alr_Clay_spec_b}\OperatorTok{$}\NormalTok{pred}\OperatorTok{$}\NormalTok{cov.pred.target)))}
\NormalTok{dvn.pred <-}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{dvr.Silt }\OperatorTok{+}\StringTok{ }\NormalTok{dvr.Clay}
...
\NormalTok{spatialpredsa}\OperatorTok{$}\NormalTok{Clay <-}\StringTok{ }\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_lab_a_map}\OperatorTok{$}\NormalTok{pred)}\OperatorTok{/}\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_lab_a_map}\OperatorTok{$}\NormalTok{pred) }\OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_lab_a_map}\OperatorTok{$}\NormalTok{pred))}
\NormalTok{spatialpredsa}\OperatorTok{$}\NormalTok{Silt <-}\StringTok{ }\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_lab_a_map}\OperatorTok{$}\NormalTok{pred)}\OperatorTok{/}\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_lab_a_map}\OperatorTok{$}\NormalTok{pred) }\OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_lab_a_map}\OperatorTok{$}\NormalTok{pred))}
...
\NormalTok{spatialpredsb}\OperatorTok{$}\NormalTok{Clay <-}\StringTok{ }\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_lab_b_map}\OperatorTok{$}\NormalTok{pred)}\OperatorTok{/}\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_lab_b_map}\OperatorTok{$}\NormalTok{pred) }\OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_lab_b_map}\OperatorTok{$}\NormalTok{pred))}
\NormalTok{spatialpredsb}\OperatorTok{$}\NormalTok{Silt <-}\StringTok{ }\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_lab_b_map}\OperatorTok{$}\NormalTok{pred)}\OperatorTok{/}\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_lab_b_map}\OperatorTok{$}\NormalTok{pred) }\OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_lab_b_map}\OperatorTok{$}\NormalTok{pred))}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{spatialpredsa}\OperatorTok{$}\NormalTok{Ca_spec <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(Ca_spec_a, }
                            \DataTypeTok{newdata =}\NormalTok{ sppx, }
                            \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtablea[variogtablea}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "Ca_spec"}\NormalTok{, varparamames]))}\OperatorTok{$}\NormalTok{pred}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{alr_Clay_spec_a_map <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Clay_spec_a, }
                               \DataTypeTok{newdata =}\NormalTok{ sppx,}
                               \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtablea[variogtablea}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Silt_spec"}\NormalTok{, varparamames]))}
\NormalTok{alr_Silt_spec_a_map <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Silt_spec_a, }
                               \DataTypeTok{newdata =}\NormalTok{ sppx, }
                               \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtablea[variogtablea}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Clay_spec"}\NormalTok{, varparamames]))}

\NormalTok{spatialpredsa}\OperatorTok{$}\NormalTok{Clay_spec <-}\StringTok{ }\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_spec_a_map}\OperatorTok{$}\NormalTok{pred)}\OperatorTok{/}\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_spec_a_map}\OperatorTok{$}\NormalTok{pred) }\OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_spec_a_map}\OperatorTok{$}\NormalTok{pred))}
\NormalTok{spatialpredsa}\OperatorTok{$}\NormalTok{Silt_spec <-}\StringTok{ }\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_spec_a_map}\OperatorTok{$}\NormalTok{pred)}\OperatorTok{/}\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_spec_a_map}\OperatorTok{$}\NormalTok{pred) }\OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_spec_a_map}\OperatorTok{$}\NormalTok{pred))}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{spatialpredsb}\OperatorTok{$}\NormalTok{Ca_spec <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(Ca_spec_b, }
                                 \DataTypeTok{newdata =}\NormalTok{ sppx, }
                                 \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtableb[variogtableb}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "Ca_spec"}\NormalTok{, varparamames]))}\OperatorTok{$}\NormalTok{pred}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{alr_Clay_spec_b_map <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Clay_spec_b, }
                               \DataTypeTok{newdata =}\NormalTok{ sppx,}
                               \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtableb[variogtableb}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Silt_spec"}\NormalTok{, varparamames]))}
\NormalTok{alr_Silt_spec_b_map <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(alr_Silt_spec_b, }
                               \DataTypeTok{newdata =}\NormalTok{ sppx,}
                               \DataTypeTok{param =} \KeywordTok{unlist}\NormalTok{(variogtableb[variogtableb}\OperatorTok{$}\NormalTok{properties }\OperatorTok{==}\StringTok{ "alr_Clay_spec"}\NormalTok{, varparamames]))}

\NormalTok{spatialpredsb}\OperatorTok{$}\NormalTok{Clay_spec <-}\StringTok{ }\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_spec_b_map}\OperatorTok{$}\NormalTok{pred)}\OperatorTok{/}\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_spec_b_map}\OperatorTok{$}\NormalTok{pred) }\OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_spec_b_map}\OperatorTok{$}\NormalTok{pred))}
\NormalTok{spatialpredsb}\OperatorTok{$}\NormalTok{Silt_spec <-}\StringTok{ }\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_spec_b_map}\OperatorTok{$}\NormalTok{pred)}\OperatorTok{/}\NormalTok{(}\DecValTok{1} \OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Silt_spec_b_map}\OperatorTok{$}\NormalTok{pred) }\OperatorTok{+}\StringTok{ }\KeywordTok{exp}\NormalTok{(alr_Clay_spec_b_map}\OperatorTok{$}\NormalTok{pred))}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## compute the differences between maps (layer)}
...
\NormalTok{## compute the differences between maps (layer B)}
...
\StringTok{  }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{strip.background =} \KeywordTok{element_rect}\NormalTok{(}\DataTypeTok{fill =} \StringTok{"grey"}\NormalTok{), }
        ...
        \DataTypeTok{strip.text.y =} \KeywordTok{element_text}\NormalTok{( }\DataTypeTok{size =} \DecValTok{25}\NormalTok{, }\DataTypeTok{colour =} \StringTok{"black"}\NormalTok{, }\DataTypeTok{angle =} \DecValTok{-90}\NormalTok{)) }
...
\NormalTok{Define palette of color }\ControlFlowTok{for}\NormalTok{ the plot}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{ca_map <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(finalmapping[finalmapping}\OperatorTok{$}\NormalTok{method }\OperatorTok{!=}\StringTok{ "Differences between maps"}\NormalTok{,], }
                 \KeywordTok{aes}\NormalTok{(POINT_X, POINT_Y)) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_tile}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{fill =}\NormalTok{ Ca)) }\OperatorTok{+}\StringTok{ }
\StringTok{  }\NormalTok{gfg }\OperatorTok{+}\StringTok{ }\NormalTok{gscf }\OperatorTok{+}\StringTok{ }\NormalTok{gcoord }\OperatorTok{+}\StringTok{ }\NormalTok{gtheme }\OperatorTok{+}\StringTok{ }\NormalTok{glabs }\OperatorTok{+}
\StringTok{  }\KeywordTok{guides}\NormalTok{(}\DataTypeTok{fill =} \KeywordTok{guide_colourbar}\NormalTok{(}\DataTypeTok{title =} \KeywordTok{expression}\NormalTok{(}\KeywordTok{paste}\NormalTok{(}\StringTok{""}\NormalTok{, Ca}\OperatorTok{^}\StringTok{"++"}\NormalTok{, }\StringTok{" "} \OperatorTok{/}\NormalTok{mmol[c], kg}\OperatorTok{^-}\DecValTok{1}\NormalTok{, }\StringTok{" "}\NormalTok{))))}
...
\NormalTok{ca_map_diff <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(finalmapping[finalmapping}\OperatorTok{$}\NormalTok{method }\OperatorTok{==}\StringTok{ "Differences between maps"}\NormalTok{,], }
                      \KeywordTok{aes}\NormalTok{(POINT_X, POINT_Y)) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_tile}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{fill =}\NormalTok{ Ca)) }\OperatorTok{+}\StringTok{ }
\StringTok{  }\NormalTok{gfg }\OperatorTok{+}\StringTok{ }\NormalTok{gscf }\OperatorTok{+}\StringTok{ }\NormalTok{gcoord }\OperatorTok{+}\StringTok{ }\NormalTok{gtheme }\OperatorTok{+}\StringTok{ }\NormalTok{glabs }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{guides}\NormalTok{(}\DataTypeTok{fill =} \KeywordTok{guide_colourbar}\NormalTok{(}\DataTypeTok{title =} \KeywordTok{expression}\NormalTok{(}\KeywordTok{paste}\NormalTok{(}\StringTok{"Difference"}\NormalTok{, }\StringTok{" "} \OperatorTok{/}\NormalTok{mmol[c], kg}\OperatorTok{^-}\DecValTok{1}\NormalTok{, }\StringTok{" "}\NormalTok{))))}
...
\NormalTok{Clay_map <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(finalmapping[finalmapping}\OperatorTok{$}\NormalTok{method }\OperatorTok{!=}\StringTok{ "Differences between maps"}\NormalTok{,], }
                 \KeywordTok{aes}\NormalTok{(POINT_X, POINT_Y)) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_tile}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{fill =}\NormalTok{ Clay)) }\OperatorTok{+}\StringTok{ }
\NormalTok{gfg }\OperatorTok{+}\StringTok{ }\NormalTok{gscf }\OperatorTok{+}\StringTok{ }\NormalTok{gcoord }\OperatorTok{+}\StringTok{ }\NormalTok{gtheme }\OperatorTok{+}\StringTok{ }\NormalTok{glabs }\OperatorTok{+}
\KeywordTok{guides}\NormalTok{(}\DataTypeTok{fill =} \KeywordTok{guide_colourbar}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Clay content, % "}\NormalTok{)) }
...
\NormalTok{Clay_map_diff <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(finalmapping[finalmapping}\OperatorTok{$}\NormalTok{method }\OperatorTok{==}\StringTok{ "Differences between maps"}\NormalTok{,], }
                      \KeywordTok{aes}\NormalTok{(POINT_X, POINT_Y)) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_tile}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{fill =}\NormalTok{ Clay)) }\OperatorTok{+}\StringTok{ }
\NormalTok{gfg }\OperatorTok{+}\StringTok{ }\NormalTok{gscf }\OperatorTok{+}\StringTok{ }\NormalTok{gcoord }\OperatorTok{+}\StringTok{ }\NormalTok{gtheme }\OperatorTok{+}\StringTok{ }\NormalTok{glabs }\OperatorTok{+}\StringTok{ }
\KeywordTok{guides}\NormalTok{(}\DataTypeTok{fill =} \KeywordTok{guide_colourbar}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Difference, %"}\NormalTok{))}
...
\NormalTok{Silt_map <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(finalmapping[finalmapping}\OperatorTok{$}\NormalTok{method }\OperatorTok{!=}\StringTok{ "Differences between maps"}\NormalTok{,], }
                 \KeywordTok{aes}\NormalTok{(POINT_X, POINT_Y)) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_tile}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{fill =}\NormalTok{ Silt)) }\OperatorTok{+}\StringTok{ }
\NormalTok{gfg }\OperatorTok{+}\StringTok{ }\NormalTok{gscf }\OperatorTok{+}\StringTok{ }\NormalTok{gcoord }\OperatorTok{+}\StringTok{ }\NormalTok{gtheme }\OperatorTok{+}\StringTok{ }\NormalTok{glabs }\OperatorTok{+}
\KeywordTok{guides}\NormalTok{(}\DataTypeTok{fill =} \KeywordTok{guide_colourbar}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Silt content, % "}\NormalTok{)) }
...
\NormalTok{Sand_map <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(finalmapping[finalmapping}\OperatorTok{$}\NormalTok{method }\OperatorTok{!=}\StringTok{ "Differences between maps"}\NormalTok{,], }
                 \KeywordTok{aes}\NormalTok{(POINT_X, POINT_Y)) }\OperatorTok{+}\StringTok{ }\KeywordTok{geom_tile}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{fill =}\NormalTok{ Sand)) }\OperatorTok{+}\StringTok{ }
\NormalTok{gfg }\OperatorTok{+}\StringTok{ }\NormalTok{gscf }\OperatorTok{+}\StringTok{ }\NormalTok{gcoord }\OperatorTok{+}\StringTok{ }\NormalTok{gtheme }\OperatorTok{+}\StringTok{ }\NormalTok{glabs }\OperatorTok{+}
\KeywordTok{guides}\NormalTok{(}\DataTypeTok{fill =} \KeywordTok{guide_colourbar}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Sand content, % "}\NormalTok{)) }
